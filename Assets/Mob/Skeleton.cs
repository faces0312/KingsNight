...
public class Skeleton : MonoBehaviour
{
    ...
    void Start()
    {
        hp = Data.Instance.gameData.skeletonHp;
        nowHp = hp;
    ...
    void Update()
    {
        if (nowHp <= 0)
        {
            Data.Instance.gameData.cutcount+=10;
    ...
    private void OnEnable()
    {
        ...
        hp = Data.Instance.gameData.michaelHp;
        nowHp = hp;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.transform.tag == "PlayerAttack" && gameObject.transform.tag != "NoDamage")
        {
            ...
            if (isRight == true)
            {
                transform.position = Vector2.Lerp(transform.position, new Vector2(-30, 10), 2 * Time.deltaTime);
            }
            if (isLeft == true)
            {
                transform.position = Vector2.Lerp(transform.position, new Vector2(30, 10), 2 * Time.deltaTime);
        ...
        if (other.transform.tag == "PlayerSkill" && gameObject.transform.tag != "NoDamage")
        {
            ...
            if (isRight == true)
            {
                transform.position = Vector2.Lerp(transform.position, new Vector2(-40, 10), 5 * Time.deltaTime);
            }
            if (isLeft == true)
            {
                transform.position = Vector2.Lerp(transform.position, new Vector2(40, 10), 5 * Time.deltaTime);
